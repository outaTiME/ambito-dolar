...
- **A restore does hand out new ids**, so `onRestored` moves the config keys across and is not dead code. `allowBackup` is false in `packages/client/app.config.ts`, which used to settle it, but from android 12 that flag no longer covers a device to device transfer: that one is `dataExtractionRules`, there is none, so on the manufacturers that still allow the transfer the default applies and the widgets come across with new ids.
- **Redraw triggers**: `APPWIDGET_UPDATE`, `MY_PACKAGE_REPLACED`, `LOCALE_CHANGED` and `TIMEZONE_CHANGED`, all through `goAsync` and all hitting `/fetch`, plus `reloadWidgets()` from `withRateUpdates` and the config save. The locale and zone ones matter because separators and time are read at render, so without them a change leaves the widget wrong for half an hour.
