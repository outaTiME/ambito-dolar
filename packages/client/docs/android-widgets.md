...
- **Nothing on disk is the other case.** A failed fetch with no stored payload leaves nothing to protect, so the widgets draw the empty text instead of keeping the bare `initialLayout`, which has no text and not even a tap. That is a first run with no network, and it is what ios shows there. Verified on a device: data cleared, booted with no network, all three drew the empty text.
- **A failed fetch leaves the widget as it was**: `render` returns early and the launcher keeps the last views it got. The one exception is the empty state below, which only draws while this provider has put nothing on a widget yet. ios does the same through a `UserDefaults` fallback in `getRates()`.
