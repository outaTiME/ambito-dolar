...
- **There is no restore path.** `allowBackup` is false in `packages/client/app.config.ts`, so a widget is never handed a new id and `onRestored` cannot fire. Config keys are written under the id the launcher gave and cleared in `onDeleted`.
